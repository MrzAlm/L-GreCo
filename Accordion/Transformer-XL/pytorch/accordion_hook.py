...
#             state.accordion_param = state.opt_compress_param
            ...
            for bucket_state in state.grad_old:
                for layer_idx in range(len(bucket_state)):
                    ...
                    if ratio < state.eta:
                        state.accordion_param[bucket_idx][layer_idx] = state.min_rank
                    else:
                        state.accordion_param[bucket_idx][layer_idx] = state.max_rank
...
#         print("optimal compression parameters = ", state.opt_compress_param)
    ...
    if state.iter_rel % state.adjust_freq == 0 and bucket_index == 0 and state.iter_rel > 0:
        ...
        state.opt_compress_param = state.accordion_param
        if dist.get_rank() == 0:
            print("optimal compression parameters = ", state.opt_compress_param)
