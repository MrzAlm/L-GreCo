...
class PowerSGDState(object):
    ...
    def __init__(
        ...
        self.acc_iter = 10
        self.eta = eta
        self.accordion_min = accordion_min
        self.accordion_max = accordion_max
        self.grad_old = []
        self.grad_new = []
        ...
        if state.iter_rel % state.adjust_freq == 0 and bucket_index == 0:
            ...
            for bucket_state in state.acc_grad:
                for layer_idx in range(len(bucket_state)):
                    ...
                    state.grad_old[bucket_idx][layer_idx] = state.grad_new[bucket_idx][layer_idx]
                    state.grad_new[bucket_idx][layer_idx] = torch.norm(state.acc_grad[bucket_idx][layer_idx]).item()
                bucket_idx += 1
            state.acc_grad = []
            
            ratio_list = []
#             state.accordion_param = state.opt_compress_param
            bucket_idx = 0
            for bucket_state in state.grad_old:
                for layer_idx in range(len(bucket_state)):
                    old_norm = state.grad_old[bucket_idx][layer_idx]
                    new_norm = state.grad_new[bucket_idx][layer_idx]
                    if old_norm != 0.0:
                        ratio = (abs(old_norm - new_norm))/(old_norm)
                    else:
                        ratio = 9
                    ...
                    if ratio < state.eta:
                        state.accordion_param[bucket_idx][layer_idx] = state.accordion_min 
                    else:
                        state.accordion_param[bucket_idx][layer_idx] = state.accordion_max
                        
                bucket_idx += 1
                        
            print("ratio list", ratio_list)
            print("accordion ranks", state.accordion_param)
    ...
    if state.iter_rel == 0:
        ...
        for tensor in tensors_to_compress:
            ...
            accordion_param.append(state.max_rank)
            grad_old.append(0.0)
            grad_new.append(0.0)
        ...
        state.accordion_param.append(accordion_param)
        state.grad_old.append(grad_old)
        state.grad_new.append(grad_new)
    ...
    if state.iter_rel % state.adjust_freq == 0 and dist.get_rank() == 0 and bucket_index == 0 and state.iter_rel > 0:
        ...
        state.compression_errors = np.zeros([num_layers, rank_range])
        state.compressed_sizes = np.zeros([num_layers, rank_range])

        i = 0
        for tensor in tensors_to_adjust:
            n, m = tensor.shape
            r = min(n, m, max_rank)
            state.compressed_sizes[i, :r - min_rank + 1] = np.arange(min_rank, r + 1) * (n + m)
            # why "+1"? so that the algorithm returns the true rank!
            state.compressed_sizes[i, r - min_rank + 1:] = r * (n + m) + 1
            ...
            if state.error_method == 'SVD':
                ...
                state.compression_errors[i, :] = torch.sqrt(torch.matmul(ut, torch.t(ss))).cpu()[min_rank - 1:r]
            ...
            if state.error_method == 'Power':
                for r in range(min_rank, min(n, m, max_rank) + 1):
                    ...
                    state.compression_errors[i, r - min_rank] = torch.dist(t, tensor)
            i += 1

        ## Discretize the Error Table
        static_error = 0
        accordion_flat = [rank for bucket_state in state.accordion_param for rank in bucket_state]
        i = 0
        for tensor in tensors_to_adjust:
            static_error += state.compression_errors[i, accordion_flat[i] - state.min_rank]
            i += 1
            
        target_score = state.alpha * static_error

        ## Fixing the NaN Problem;
        state.compression_errors = np.minimum(state.compression_errors, target_score + 1)
        np.nan_to_num(state.compression_errors, nan=target_score + 1)

        bucket_size = target_score / DPBUCKETS
        state.compression_errors = np.minimum(np.maximum(np.ceil(state.compression_errors / bucket_size), 1), DPBUCKETS)

        np.nan_to_num(state.compression_errors, nan=DPBUCKETS + 1)
        state.compression_errors = state.compression_errors.astype(int)
        ...
        for bucket_idx in range(num_buckets):
            for val_idx in range(num_values):
                if bucket_idx > state.compression_errors[0, val_idx]:
                    tmp = state.compressed_sizes[0, val_idx]
        ...
        for layer_idx in range(1, len(DP)):
            for val_idx in range(num_values):
                comp_size = state.compressed_sizes[layer_idx][val_idx]
                comp_error = state.compression_errors[layer_idx][val_idx]
        ...
        if total_error <= DPBUCKETS:
            ...
            for layer_idx in range(len(DP) - 1, -1, -1):
                ...
                total_error -= state.compression_errors[layer_idx, PD[layer_idx][total_error]]
